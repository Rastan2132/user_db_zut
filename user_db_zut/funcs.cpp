...
void show(Users_book* pb, short size)
{
	...
	{
		// if (i < 9) cout << "0";
		cout << right << setw(3) << setfill('0') << i + 1 << setfill(' ') << " " << MANIP << pb[i].Name << " " << MANIP << pb[i].Surname << "  " << MANIP << pb[i].Year << "\n";
	}
}
...
Users_book* add(Users_book* pb, short* size)
{
	...
	cout << endl;
	record(pb[(*size) - 1].Year, 3, " Year: ", false, 1, false);
	cout << endl;
	return pb;
}
...
Users_book* del(Users_book* pb, short* size)
{
	...
	if (*size > 0)
	{
		cout << "NUMER UZYTKOWNIKA DO USUNIECIA" << endl << endl;
		cin >> index;
		pb = resize(pb, size, (*size - 1), index);
...
Users_book* init(Users_book* pb, short* size)
{
	...
	while (!in.eof())
	{
		pb = resize(pb, size, (*size) + 1);
		i = *size - 1;
		in >> pb[i].Name >> pb[i].Surname >> pb[i].Year;
	}
...
bool save(Users_book* pb, short* size)
{
	...
	{
		out << pb[i].Name << " " << pb[i].Surname << " " << pb[i].Year;
